...
static __init const char *early_boot_devpath(const char *initial_devname)
{
	...
	if (err < 0)
		pr_err(" init_mkdir err =%d", err);

	err = init_unlink(devname);
	if (err < 0)
		pr_err(" init_unlink err =%d", err);

	err = init_mknod(devname, S_IFBLK | 0600, new_encode_dev(dev));
	if (err < 0)
		pr_err(" init_mknod err =%d", err);
...
static int __init __best_effort_init(void)
{
	...
	if (ret)
		...
		pr_err("attached %s (%lu) (no dedicated panic_write!)\n",
